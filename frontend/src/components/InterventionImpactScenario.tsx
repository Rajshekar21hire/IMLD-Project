...
export const InterventionImpactScenario: React.FC = () => {
  ...
  const raiseWhoLabel = currentPct - whoPct > 5;
  const whoLabelLeft = Math.min(100, Math.max(7, whoPct + 1.5));

  return (
    <div className="s4h-root mx-auto w-full max-w-[170rem] px-6 md:px-10">
      <div className="text-center">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#0C447C]">Intervention scenario</p>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-semibold transition-colors ${
                on ? 'border-indigo-500 bg-indigo-500' : 'border-blue-200 bg-white text-slate-600 hover:border-indigo-300'
              }`}
              style={{ color: on ? '#ffffff' : '#475569' }}
            >
              <span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: on ? '#ffffff' : tierColor }} />
              {c.name}
            </button>
          );
        })}
      ...
      <div className="mt-8">
        ...
        <div className="relative mt-1 h-12 text-xs text-slate-500">
          <span
            className={`absolute -translate-x-1/2 ${raiseWhoLabel ? '-top-1' : 'top-4'}`}
            style={{ left: `${whoLabelLeft}%` }}
          >
            WHO guideline: {WHO_GUIDELINE}
          </span>
          <span className="absolute top-0 -translate-x-1/2" style={{ left: `${currentPct}%` }}>
            Current: {city.pm25}
          </span>
        </div>
      </div>
