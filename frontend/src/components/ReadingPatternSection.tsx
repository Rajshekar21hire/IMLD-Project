            ...
            {steps.map((step, index) => (
              <React.Fragment key={step.title}>
                <article className={`rounded-2xl rounded-t-none border border-slate-200 border-t-4 ${step.accent} bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-sky-400 hover:shadow-md`}>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-sky-700 align-justify">{step.eyebrow}</p>
                  <h3 className="mt-2 text-[1.05rem] font-semibold text-slate-950">{step.title}</h3>
                  <p className="mt-3 text-sm leading-relaxed text-slate-700 align-justify">{step.body}</p>
                </article>
