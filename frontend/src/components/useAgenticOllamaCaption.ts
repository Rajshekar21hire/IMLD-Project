...
const inflightCaptions = new Map<string, Promise<string>>();
...
export function useAgenticOllamaCaption(userMessage: string | null, fallback: string, debounceMs = 120) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      (async () => {
        ...
        const messageKey = userMessage.trim();

        const requestPromise = (() => {
          const existing = inflightCaptions.get(messageKey);
          if (existing) return existing;
          const created = storyAPI
            .agenticCaption({ message: userMessage })
            .then((res) => {
            ...
            .finally(() => {
              inflightCaptions.delete(messageKey);
            });
          inflightCaptions.set(messageKey, created);
          return created;
        })();
